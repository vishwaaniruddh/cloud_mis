...
if (isset($_SESSION['username'])) {
    ...
    if (mysqli_query($con, $updatesql)) {
        ...
        $insertfundsql = "INSERT INTO eng_fund_request_history (fundreq_id, requested_amount, approved_amount, action_by, status, created_at, updated_at,remarks) 
                          VALUES ('$fund_id', '$finalapproved_amount', '$approved_amt', '$action_by', 2, '$created_at', '$created_at','$fund_remarks')";
        if (mysqli_query($con, $insertfundsql)) {
            
            $additional = ' Ask : ' . $finalapproved_amount . ' & Approved : ' .  $approved_amt ; 
            
            mysqli_query($con,"update mis_details set status = 'fund_level_2_pending' where id='".$mis_id."' ");
            
            $statement = "INSERT INTO mis_history (mis_id, type, remark, created_at, created_by) 
              VALUES ('$mis_id', 'Level 1 Fund Processed', '$fund_remarks .' '. $additional', '$datetime', '$userid')";
