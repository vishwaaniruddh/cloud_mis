...
$sql_query = "select a.remarks,a.id AS misid,a.bank,a.customer,a.location,a.zone,a.state,a.city,a.branch,a.created_by,a.bm,b.id,b.mis_id, b.atmid,b.component,b.subcomponent,b.engineer,b.docket_no,b.status,b.created_at,b.ticket_id,b.close_date,b.call_type,b.case_type , b.footage_requestBy,b.footage_format,b.footage_txn_time,b.footage_status, (SELECT CONCAT(name) from mis_loginusers WHERE id= b.engineer) AS eng_name, (SELECT CONCAT(contact) from mis_loginusers WHERE id= b.engineer) AS eng_contact, IF(b.footage_date = '0000-00-00 00:00:00', '', DATE_FORMAT(b.footage_date, '%Y-%m-%d')) AS footage_date, b.fromtime,b.totime, (SELECT name from mis_loginusers WHERE id= a.created_by) AS createdBy, cc.type,cc.schedule_date,cc.material,cc.material_condition,cc.courier_agency,cc.pod,cc.serial_number,cc.dispatch_date,cc.status_remark, cc.material_req_remark, cc.material_dispatch_remark,cc.close_remark,cc.last_action_by,cc.created_date from mis a INNER JOIN mis_details b ON b.mis_id = a.id LEFT JOIN (select c.mis_id as cmisid,c.type,c.schedule_date,c.material,c.material_condition,c.courier_agency,c.pod,c.serial_number,c.dispatch_date,c.remark as status_remark, IF(c.type='material_requirement',c.remark,'') as material_req_remark, IF(c.type='material_dispatch',c.remark,'') as material_dispatch_remark, IF(c.type='close',c.remark,'') as close_remark, c.created_by as last_action_by, c.created_at as created_date from mis_history c group by c.mis_id ) AS cc ON cc.cmisid = a.id where 1 and a.customer in('Euronet', 'Diebold', 'FSS', 'HITACHI', 'AGS', 'TATA') and b.mis_id = a.id and b.branch in('Bhubaneshwar', 'Bihar', 'Guwahati', 'Jharkhand', 'Kolkata', 'Patna', 'Agra', 'Ajmer', 'Allahabad', 'Alwar', 'Bareilly', 'Bhiwandi', 'Bikaner', 'Dehradun', 'Delhi', 'Gorakhpur', 'Haryana', 'Jaipur', 'Jhansi', 'Jodhpur', 'Kota', 'Lucknow', 'Ludhiana', 'Meerut', 'Mirzapur', 'Sikar ', 'Udaipur', 'Varanasi', 'Bangalore', 'Chennai', 'Hyderabad', 'Hyderabad 1', 'Kerala', 'Ahmedabad', 'Ahmednagar', 'Amravati', 'Aurangabad', 'Bhopal', 'Goa', 'Gwalior', 'Indore', 'Jabalpur', 'Jalgaon', 'Kolhapur', 'Mumbai', 'Nagpur', 'Nanded', 'Nashik', 'Pune', 'Raipur', 'Rajkot', 'Ratnagiri', 'Solapur', 'Surat', 'Vadodara', 'Vapi') and b.zone in('East', 'North', 'South', 'West') and b.call_type in('Project', 'Service', 'Footage') and a.bank in('HDFC', 'IndusInd', 'RBL', 'AXIS', 'ICICI', 'SBI', 'DBS', 'IDFC', 'BOI', 'PNB', 'Kotak', 'YES', 'DLB', 'WLA', 'IDBI', 'Punjab Nat', 'HDFC Bank', 'Axis Bank', 'Bajaj Fina', 'MONEY SPOT', 'State Bank', 'IndusInd B', 'YES Bank', 'Dhanlaxmi ', 'ICICI Bank', 'Punjab & S', 'RATNAKAR B', 'Kotak Mahi', 'Bank of In', 'Bank of Ba', 'SBI TOM 2', 'Writer', 'RSBL') and b.status in('open', 'schedule') and CAST(b.created_at AS DATE) >= '2023-01-01' and CAST(b.created_at AS DATE) <= '2025-03-25' order by b.id desc LIMIT 1 ";
...
foreach ($sql_result as $key => $value) {
    if (is_null($value)) {
        ...
        $encoding = mb_detect_encoding($value, ["UTF-8", "ISO-8859-1", "WINDOWS-1252"]);
        echo "$key => $value (Encoding: $encoding) \n";
    }
}
echo '</pre>';
...
// echo '<br />';
// var_dump($lastactionsql_result);
echo '<br />';
echo '<br />';


echo '<pre>'; // Format output for readability
foreach ($lastactionsql_result as $key => $value) {
    if (is_null($value)) {
        echo "$key => NULL \n"; // Show NULL values
    } elseif (is_array($value)) {
        echo "$key => ARRAY \n"; // If it's another array
...
echo '<pre>'; // Format output for readability
foreach ($matConditionResult as $key => $value) {
    if (is_null($value)) {
        echo "$key => NULL \n"; // Show NULL values
    } elseif (is_array($value)) {
        echo "$key => ARRAY \n"; // If it's another array
...
$clean_location = preg_replace('/[^\x20-\x7E]/', '', $sql_result['location']); // Remove non-ASCII
$sheet->setCellValue('F' . $i, $clean_location);
