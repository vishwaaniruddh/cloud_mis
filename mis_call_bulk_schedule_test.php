					...
					<div class="page-body">
						<div class="card">
							<div class="card-block">
								<div class="two_end">
									<h5>Call Bulk Schedule Test <span style="font-size:12px; color:red;">(Bulk Upload)</span></h5> <a class="btn btn-success" href="excelformat/mis_call_bulk_schedule.xlsx" download>Call Bulk Schedule Format</a> </div>
								<?php 
                                              ...
                                              $ticketid = $rowData[$i][0][0];
                                                if($ticketid){
                                                  ...
                                                  //  $datetime = date('Y-m-d h:i:s');
                                                        ...
                                                        $search = mysqli_query($con,"select * from mis_details_test where ticket_id = '".$ticketid."' ");
                                                        $search_rows = mysqli_num_rows($search);
                                                        $search_result = mysqli_fetch_assoc($search);
                                                        $misid = $search_result['mis_id'];
                                                        $misdetails_id = $search_result['id'];
                                                        
                                                        
                                                        
                                                        $update = mysqli_query($con,"update mis_details_test set status='".$status."',engineer = '".$eng_id."' where mis_id='".$misid."' ");
                                                        // var_dump($update);
                                                        if($update)
                                                        {
                                                            $updatemis = mysqli_query($con,"update mis_test set remarks='$remark' where id='$misid' ");
                                                            
                                                            $insertqry = mysqli_query($con,"insert into mis_history_test(mis_id,type,schedule_date,engineer,remark,status,created_at,created_by)values('".$misdetails_id."','".$status."','".$schedule_date."','".$eng_id."','".$remark."',1,'".$created_at."','".$userid."')");
                                                        }
									...
									<table class="table table-bordered table-striped" id="data_table" style="width:100%;">
										...
										<tbody>
											<?
                                                ...
                                                $scheduleData = mysqli_query($con,"select * from mis_history_test where mis_id='".$misid."' and status=1  order by id desc");
                                                ...
                                                $sqldata = mysqli_query($con,"select * from mis_details_test where mis_id='".$misid."' ");
                                            ...
                                            ?>
												<tr>
													...
													<td>
														<?php echo $scheduleData_result['schedule_date'] ; ?>
													</td>
													<td>
														<?php echo $engData_result['name'] ; ?>
													</td>
													<td>
														<?php echo $scheduleData_result['remark'] ; ?>
													</td>
