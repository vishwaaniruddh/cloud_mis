...
if ($_SESSION['username']) {
    ...
    {
        global $con;
        $sql = mysqli_query($con, "select $parameter from mis_history_test where type='" . $type . "' and mis_id='" . $id . "'");
...
if($callTypePermission){
        ...
        if (isset($_REQUEST['submit']) || isset($_GET['page'])) { 
                ...
                b.footage_requestBy,b.footage_format,b.footage_txn_time,b.footage_status,
                ...
                from mis_test a
                    INNER JOIN mis_details_test b ON b.mis_id = a.id 
                    ...
                    from mis_history_test c group by c.mis_id
                    
                    ) AS cc  ON cc.cmisid = a.id 
                    
                    where 1 and 
                a.customer in($assigned_customer) and
                b.mis_id = a.id $branch_query $zone_query $callTypeQuery $bankQuery
                ";
                ...
                if (isset($_REQUEST['status']) && $_REQUEST['status'] != '') {
                    ...
                    $statement .= " and b.status in('open','permission_require','dispatch','material_requirement','material_in_process','schedule','material_available_i','material_dispatch','cancelled','not_available','available','close','MRS','fund_required','service_center','Pending')";
                    $sqlappCount .= " and b.status in('open','permission_require','dispatch','material_requirement','material_in_process','schedule','material_available_i','material_dispatch','cancelled','not_available','available','close','MRS','fund_required','service_center','Pending')";
                }
                ...
                if ($_REQUEST['atmid'] == '' && $_REQUEST['customer'] == ''  &&  $_REQUEST['call_type']=='' && $_REQUEST['engineer']=='') { 
                    ...
                    where 1 and
                        ...
                        a.customer in($assigned_customer) and
                        b.mis_id = a.id $branch_query $zone_query $callTypeQuery $bankQuery and
                        b.status in($status) ";
                                    ...
                                    <div class="row">
                                         ...
                                         <div class="col-sm-3">
                                            ...
                                            <select name="call_type" id="call_type" class="form-control">
                                                ...
                                                while($callTypesql_result = mysqli_fetch_assoc($callTypesql)){
                                                    ...
                                                    $call_type_val = $callTypesql_result['call_type'];
                                                    
                                                     if(in_array($callTypeId,$callTypePermission_AR)){ ?>
                                                            <option value="<?php echo $call_type_val ;?>" >
                                                                <?php echo $call_type_val ;?>
                                        ...
                                        <div class="col-sm-3">
                                            <label>Banks</label>
                                              ...
                                              <select id="multiselect_banks" class="form-control" name="bank[]" multiple="multiple">
                                                <?
                                                $i = 0;
                                                $bank_sql = mysqli_query($con, "select distinct(bank) as bank from mis_newsite where status='1'");
                                                while ($bank_sql_result = mysqli_fetch_assoc($bank_sql)) { 
                                                  
                                                  $bankName = $bank_sql_result['bank'] ; 
                                         if(in_array($bankName,$bankPermission_AR)){ ?>
                                                  
                                                ?>
                                                    <option value="<? echo $bankName; ?>" <?  if(isset($_REQUEST['bank'])) { 
                                                    
                                                    if(in_array($bankName,$_REQUEST['bank'])){
                                                        echo 'selected';
                                                        }
                                                    }
                                                    ?>
                            ...
                            <div class="card">
                                <div class="card-block">
                                    <div style="display:flex;justify-content:space-around;">
                                        <h5 style="text-align:center;">mis_test Detailed Report - <p>Total Records- <? echo $total_records ;  ?></p></h5>
                                            ...
                                            <tbody>
                                                <?php
                                                    ...
                                                    while ($sql_result = mysqli_fetch_assoc($sql_app)) {
                                                        ...
                                                        $historydate = mysqli_query($con,"select created_at from mis_history_test where mis_id='".$id."' order by id desc limit 1");
                                                        ...
                                                        $mis_his_key = 0;
                                                        echo "select type,created_by,remark,schedule_date,material,material_condition,courier_agency,pod,serial_number,dispatch_date,(SELECT name FROM mis_loginusers WHERE id=mis_history.created_by) AS last_action_by from mis_history_test where mis_id='" . $id . "' order by id desc";
                                                        
                                                        $lastactionsql = mysqli_query($con, "select type,created_by,remark,schedule_date,material,material_condition,courier_agency,pod,serial_number,dispatch_date,(SELECT name FROM mis_loginusers WHERE id=mis_history_test.created_by) AS last_action_by from mis_history_test where mis_id='" . $id . "' order by id desc");
                                                        ...
                                                        $matCondition = mysqli_query($con,"select * from mis_history_test where mis_id='".$id."' ORDER BY `mis_history`.`material_condition` DESC");
                                                ...
                                                ?>
                                                    <tr <? if ($aging_day > 3 && $status != 'close') { ?> style="background:#fe5d70c2;color:white;" <? } if ($status == 'close') { ?> style="background:#0ac282;color:white;" <?  } elseif ($status == 'schedule') {  ?> style="background:#6c757d;color:white;" <? } elseif ($status == 'open' || $status == 'Pending') {  ?> style="background:yellow;color:black;" <? }  ?>>
                                                        <!--<td><? echo ++$i; ?></td>-->
                                                        ...
                                                        <!--<td><?php echo ($schedule_remark ? $schedule_remark : '-'); ?></td>-->
                                                        <td><?php echo $schedule_remark; ?></td>
                                                        ...
                                                        <td><?php echo $sql_result['footage_status'];?></td>
                                                        ...
                                                        <td><? echo ($sql_result['footage_requestBy'] ? $sql_result['footage_requestBy']: '-'); ?></td>
                                                        
                                                        <td><? echo ($sql_result['footage_format'] ? $sql_result['footage_format']: '-'); ?></td>
                                                        
                                                        <td><? echo ($sql_result['footage_txn_time'] ? $sql_result['footage_txn_time']: '-'); ?></td>
