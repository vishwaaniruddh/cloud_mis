...
if($callTypePermission){
        ...
        if (isset($_REQUEST['submit']) || isset($_GET['page'])) { 
                ...
                if (isset($_REQUEST['status']) && $_REQUEST['status'] != '') {
                    ...
                    $statement .= " and b.status in('open','permission_require','dispatch','material_requirement','material_in_process','schedule','material_available_i','material_dispatch','cancelled','not_available','available','close','MRS','fund_required','service_center','Pending','material_request_rejected','material_request_approved','material_request_app')";
                    $sqlappCount .= " and b.status in('open','permission_require','dispatch','material_requirement','material_in_process','schedule','material_available_i','material_dispatch','cancelled','not_available','available','close','MRS','fund_required','service_center','Pending','material_request_rejected','material_request_approved','material_request_app')";
                }
                                            ...
                                            <select id="multiselect_status" class="form-control" name="status[]" multiple="multiple">
                                                ...
                                                ?>
                                                    ...
                                                    if(in_array($status_sql_result['status_code'],$_REQUEST['status'])){
                                                        ...
                                                        if($status_sql_result['status_name']!='Closed' && $status_sql_result['status_name']!='Submitted'){
                                                            echo 'selected';
                                                        }
                                                    }
                                                    ?>
                                            ...
                                            <tbody>
                                                <?php
                                                    ...
                                                    while ($sql_result = mysqli_fetch_assoc($sql_app)) {
                                                        ...
                                                        $fund_found = true ; 
                                                        // switch ($status) {
                                                        //     case $status=='fund_required':
                                                        //         $new_status_label  = 'Fund-Level 1 Pending';
                                                        //         break;
                                                        //     case $status=='fund_level_1_pending':
                                                        //         $new_status_label  = 'Fund-Level 2 Pending';
                                                        //         break;
                                                        //     case $status=='fund_level_2_pending':
                                                        //         $new_status_label  = 'Fund-Batch Pending';
                                                        //         break;
                                                        //     case $status=='fund_batch_pending':
                                                        //         $new_status_label  = 'Fund-Under Process';
                                                        //         break;
                                                        //     default:
                                                        //         $new_status_label = '';
                                                        //         $fund_found = false ; 
                                                ...
                                                ?>
                                                    <tr <? if ($aging_day > 3 && $status != 'close') { ?> style="background:#fe5d70c2;color:white;" <? } if ($status == 'close') { ?> style="background:#0ac282;color:white;" <?  } elseif ($status == 'schedule') {  ?> style="background:#6c757d;color:white;" <? } elseif ($status == 'open' || $status == 'Pending') {  ?> style="background:yellow;color:black;" <? }  ?>>
                                                        ...
                                                        <td><strong><? echo $counter; ?></strong></td>
